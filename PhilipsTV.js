...
class PhilipsTV {
    ...
    constructor(config) {
        ...
        console.log(`https://${config.ip_address}:1926/6/`);
        ...
        this.ambilight = new Ambilight(this.api);
    }
    ...
    getPictureMode = (callback) => {
        let body = {nodes: [{nodeid: 2131230838}]};
        this.api("menuitems/settings/current", body)
            .then((data) => {
                let selectedItem = data["values"][0]["value"]["data"]["selected_item"];
                callback(null, selectedItem);
            })
            .catch(() => {
                callback(null, null);
            });
    ...
    setPictureMode = (modeID, callback) => {
        let body = {
            values: [
                {
                    value: {
                        ...
                        Controllable: true,
                        Available: true,
                        data: {selected_item: modeID},
                    },
                },
            ],
        };
        this.api("menuitems/settings/update", body)
            .then((data) => {
                console.log(data);
                callback(null, data);
            })
